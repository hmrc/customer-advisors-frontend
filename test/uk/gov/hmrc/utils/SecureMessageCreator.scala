...
object SecureMessageCreator {
  ...
  val uncleanContent = new String(
    Base64.encodeBase64(
      "<p>This is the content of the secure message</p><script>alert('more hax')</script>".getBytes("UTF-8")
    )
  )
  ...
  val uncleanMessage =
    SecureMessage(Recipient(utr), externalReference, messageType, uncleanSubject, uncleanContent, validFrom, details)
}
